...
@pytest.fixture
def deep_model():
    """2x2 = 4 site model with 2 hidden layers."""
    model = MADE(n_sites=4, hidden_dims=(16, 16))
    key = jax.random.PRNGKey(42)
    params = model.init(key, jnp.ones(4))
    return model, params
...
class TestAutoregressiveProperty:
    @pytest.mark.parametrize("hidden_dims", [(16,), (16, 16), (32, 32)])
    def test_jacobian_mask(self, hidden_dims):
        """∂logit_k/∂z_j = 0 for j >= k (autoregressive constraint)."""
        N = 4
        model = MADE(n_sites=N, hidden_dims=hidden_dims)
        key = jax.random.PRNGKey(0)
        z = jax.random.choice(key, jnp.array([-1.0, 1.0]), shape=(N,))
        params = model.init(key, z)

        J = jax.jacobian(lambda z: model.apply(params, z))(z)
        assert jnp.allclose(jnp.triu(J), 0.0, atol=1e-6), \
            f"Upper triangle not zero for hidden_dims={hidden_dims}"
...
class TestNormalization:
    ...
    def test_sum_to_one_deep(self, deep_model):
        """Normalization holds for multi-layer MADE."""
        model, params = deep_model
        configs = jnp.array(
            [list(c) for c in product([-1.0, 1.0], repeat=4)]
        )
        lps = jax.vmap(lambda z: log_prob(model, params, z))(configs)
